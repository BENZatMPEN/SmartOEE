...
import { OEE_TYPE_A } from '../../../../constants';
...
type Props = {
  ...
  onUpdate: (oeeBatchA: OeeBatchA) => void;
  editingOeeBatchA: OeeBatchA;
};

export default function DashboardOperatingOeeADialog({ open, onClose, onUpdate, editingOeeBatchA }: Props) {
  ...
  const onSubmit = async (data: FormValuesProps) => {
    try {
      ...
      const response = await axios.patch<OeeBatchA>(`/oee-batches/${editingOeeBatchA.oeeBatchId}/a-param`, {
        id: editingOeeBatchA.id,
  ...
  const filterMachineParams = (machine: Machine): MachineParameter[] => {
    const filtered = machine.parameters.filter((param) => param.oeeType === OEE_TYPE_A);
