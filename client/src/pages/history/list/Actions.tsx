...
import { HISTORY_LOG_TYPE_ACTION, ROWS_PER_PAGE_OPTIONS } from '../../../constants';
...
export default function HistoryAction() {
  ...
  const handleExport = async () => {
    ...
    saveAs(new Blob([response.data]), `action-logs.xlsx`);
  };
      ...
      <Scrollbar>
        <TableContainer sx={{ minWidth: 800 }}>
          <Table size={'medium'}>
            ...
            <TableBody>
              {(isLoading ? [...Array(rowsPerPage)] : pagedList.list).map((row: HistoryLog, index) =>
                row ? (
